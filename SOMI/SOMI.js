...
function SomiAttackHook(e, v) {
	if(Player.isPlayer(e)) ModPE.showTipMessage(Somi.entity.indexOf(v) + Somi.emotion[Somi.entity.indexOf(v)]);
}
...
function SomiMainActivity() {for(var e in Somi.entity) {
	...
	switch(Somi.emotion[e]) {
		case "FOLLOW_SURVIVAL":
			...
			Entity.setVelX(Somi.entity[e], absX(px-x, py-y, pz-z)/6);
			Entity.setVelZ(Somi.entity[e], absZ(px-x, py-y, pz-z)/6);
		...
		case "IDLE":
			if(rangeEnt(Somi.entity[e], Player.getEntity()) > 8) {
				if(Level.getGameMode() == 1) {
					Somi.emotion[e] = "FOLLOW_CREATIVE";
					debug("a.i", Somi.entity[e] + "followCreative");
				}else {
					Somi.emotion[e] = "FOLLOW_SURVIVAL";
					debug("a.i", Somi.entity[e] + "followSurvival");
				}
			}
			break;
	}
